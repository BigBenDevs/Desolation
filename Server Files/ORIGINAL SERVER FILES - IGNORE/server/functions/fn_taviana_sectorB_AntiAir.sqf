scriptName "fn_taviana_sectorB_AntiAir";
/*--------------------------------------------------------------------
	Author: OPTiX (ofpectag: OPX)
    File: fn_taviana_sectorB_AntiAir.sqf
...
--------------------------------------------------------------------*/
#define __filename "fn_taviana_sectorB_AntiAir.sqf"
...
while {dz_sv_sectorB_AA_loop} do {
	...
	if (({(getMarkerPos "dz_airoampoint_0") distance _x < 2000} count AllPlayers) > 0) then {
		if (isNil "dz_sv_sectorB_AA") then {
			dz_sv_sectorB_AA = ([[22755.1,19636.4,-0.0898876], 265.64, "CUP_B_ZSU23_CDF", EAST] call BIS_fnc_spawnVehicle) select 0;
